! Populaatioon liittyviä funktioita sisältävä moduuli
module population
    implicit none
    ! Alustetaan person-tyyppi täällä jotta ei tule erroria
    type :: person
    ...
    contains
        ...
        type (person) function spreadDisease(pop, popArray, illProc, popSize)
            implicit none
            type (person), intent(in) :: pop
            ...
            ! Arvotaan henkilölle x- ja y-koordinaatit
            real :: random
            integer :: prc, i, isThereAnyoneSick ! isThereAnyoneSick asetetaan 1 jos löydetään joku sairas samasta koordinaatista
            ! Arvotaan random lukua
            call random_number(random)
            ! Arvotaan jotain 0-100 väliltä
            prc = nint(100 * random)
